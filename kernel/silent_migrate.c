...
/*
	Check if page is pinned and if true fill the address of its struct page_owner in argument pg_owner
	We set a flag in page extension to mark pinned pages
	Make sure page extension relation kconfigs are enabled
*/
bool is_page_pinned(struct page *pg,struct page_owner **pg_owner)
{
	struct page_ext *page_ext;
    struct page_owner *temp;
	//printk(KERN_INFO "is_page_pinned() entry");
...
void print_page_info(struct page *pg)
{
	...
	printk(KERN_INFO "page_count = %d and mapcount =  %d for pfn  %lu",page_count(pg),page_mapcount(pg),pfn);
...
struct dma_pte *find_page_iova_pte(struct page *pg)
{
	...
	//just a hack because I am not able to get the dma_device for vfio driver case and directly uing the domain
...
/*
	Function to migrate a page and update the IOVA mapping
	Ensure that dst page is allocated and refcount is incremented before calling this function
*/
int migrate_and_update_iova_mapping(struct dma_pte *dma_pte, struct page_owner *pg_owner, struct page *dst,struct page *src)
{	
	...
	/*
		...
		Since PTE contains page addresses that are address aligned the page offset bits VTD_PAGE_SHIFT bits are used for flag;
		last bits apart from mask is some special flag so that is also masked
	*/
	dma_pte_flag = dma_pte->val & VTD_PAGE_MASK & (~DMA_FL_PTE_XD);
	dma_pte_new.val = dma_pte_flag | (dest_pfn_val << VTD_PAGE_SHIFT);
	
	/*
		Step 1: clearing access bit for dma pte
		---------------------------------------
	*/
	...
	/*
		Step 2: Shootdown
		-----------------
		IOTLB invalidation in intel iommu is at different granularity: DMA_TLB_GLOBAL_FLUSH,DMA_TLB_DSI_FLUSH-domain selective invalidation, DMA_TLB_PSI_FLUSH-page selective invalidation.
		Naively I am going with DMA_TLB_GLOBAL_FLUSH
		Internally intel tlb flushing logic calls below function 
		static void __iommu_flush_iotlb(struct intel_iommu *iommu, u16 did,u64 addr, unsigned int size_order, u64 type)
		set did=0 if domain flushing function not present in hardware . I think that is what I read. Check again
	*/
	//intel specific because of intel_iommu struct is used
	if(pg_owner->dma_device!=NULL)
	{
		...
		/*
		third last parametr is address, since it is global invalidation putting a zero but what if it was psi, what address would go here
		*/
		printk(KERN_INFO " IOTLB Flush\n");
		iommu->flush.flush_iotlb(iommu,0,0,0,DMA_TLB_GLOBAL_FLUSH); 
	}
	//intel iommu agnostic code
	...
	{
		printk(KERN_WARNING "Both IOMMU struct and domain are NULL, no migration, returning\n");
		return 0;
	}
	...
	*/
	...
	//Assumed that the src page had only one userspace mapping
	//If mapcount of src page is more than 1 then we should be handle that case ideally but for simplicity we assume single mapping
...
/*
	Function to migrate pinned pages
*/
	...
	*/
	...
	{	
		...
		/*
			...
			Remember sometimes applications can pin pages that are mapped to a virtual file
		*/
		/*
		...
		*/
		...
		/*
			Enable this condition to skip stack pages
		*/
		/*
		...
		*/
		...
		{	
			...
			*/
			...
			/*
				PagePrivate() probably means that we do not have a struct page for a physical page, so skip those
			*/
			if(PagePrivate(src))
				continue;
